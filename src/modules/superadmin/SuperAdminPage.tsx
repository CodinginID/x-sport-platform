...
function AddonConfigCard({ adminClient, config, onConfigChange, toast }: {
  ...
  const save = async () => {
    setSaving(true);
    const { error } = await adminClient.from('platform_config').update({
      admin_wa: cfg.admin_wa,
      bank_name: cfg.bank_name,
      bank_account_number: cfg.bank_account_number,
      bank_account_holder: cfg.bank_account_holder,
      feature_prices: cfg.feature_prices,
      updated_at: new Date().toISOString(),
    }).eq('id', 1);
    setSaving(false);
    toast.add(error ? 'Gagal simpan: ' + error.message : 'Pengaturan add-on tersimpan', error ? 'error' : 'success');
  };
  ...
  return (
    <div className="glass-card rounded-3xl p-6 space-y-4">
      <h2 className="text-sm font-bold flex items-center gap-2"><Sparkles size={15} className="text-zen-brand" /> Pengaturan Add-on</h2>

      <div className="grid sm:grid-cols-2 gap-3">
        <div>
          <label className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40 mb-1.5 block">Nomor WA Admin</label>
          <input className={inputCls} placeholder="628123456789" value={cfg.admin_wa} onChange={e => setField('admin_wa', e.target.value)} />
        </div>
        <div>
          <label className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40 mb-1.5 block">Nama Bank</label>
          <input className={inputCls} placeholder="BCA" value={cfg.bank_name} onChange={e => setField('bank_name', e.target.value)} />
        </div>
        <div>
          <label className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40 mb-1.5 block">No. Rekening</label>
          <input className={inputCls} placeholder="1234567890" value={cfg.bank_account_number} onChange={e => setField('bank_account_number', e.target.value)} />
        </div>
        <div>
          <label className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40 mb-1.5 block">Atas Nama</label>
          <input className={inputCls} placeholder="Nama pemilik" value={cfg.bank_account_holder} onChange={e => setField('bank_account_holder', e.target.value)} />
        </div>
      </div>
      ...
      <button onClick={save} disabled={saving}
        className="px-5 py-3 rounded-2xl bg-zen-brand text-white text-sm font-bold disabled:opacity-50">
