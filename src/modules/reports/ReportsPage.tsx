...
export default function ReportsPage() {
  ...
  const tabs = [t('reports.sales'), t('reports.member_payments'), t('reports.member_detail'), t('reports.member_balance'), t('reports.coach_commission'), t('reports.profit')] as const;
  ...
  const printSalesReport = async () => {
    const doc = await generateReport(`${t('reports.sales')} (${formatDate(startDate)} - ${formatDate(endDate)})`, [t('bookings.date'), t('payments.customer'), 'Items', t('payments.total')],
      ...
      [{ label: t('payments.total'), value: formatCurrency(sales.reduce((sum, s) => sum + s.total, 0)) }]);
    setPdfUrl(previewPdf(doc)); setPdfFilename('laporan-penjualan.pdf');
  };
  const printPaymentsReport = async () => {
    const doc = await generateReport(`${t('reports.member_payments')} (${formatDate(startDate)} - ${formatDate(endDate)})`, [t('bookings.date'), 'Member', t('packages.title'), t('payments.amount'), t('payments.method')],
      ...
      [{ label: t('payments.total'), value: formatCurrency(payments.reduce((sum, p) => sum + p.amount, 0)) }]);
    setPdfUrl(previewPdf(doc)); setPdfFilename('laporan-pembayaran.pdf');
  };
  const printBalanceReport = async () => {
    const doc = await generateReport(t('reports.member_balance'), ['Member', t('packages.title'), t('reports.remaining_sessions'), t('payments.total'), t('bookings.status'), t('reports.expired')],
  ...
  const printCommissionReport = async () => {
    const doc = await generateReport(`${t('reports.coach_commission')} (${formatDate(startDate)} - ${formatDate(endDate)})`, [t('bookings.date'), t('commissions.coach'), 'Member', t('packages.price'), '%', t('commissions.amount')],
      ...
      [{ label: t('commissions.total'), value: formatCurrency(commissions.reduce((sum, c) => sum + c.commission_amount, 0)) }]);
  ...
  const printProfitReport = async () => {
    ...
    const doc = await generateReport(`${t('reports.profit')} (${formatDate(startDate)} - ${formatDate(endDate)})`, ['', t('payments.amount')],
      [[t('reports.member_income'), formatCurrency(tp)], [t('reports.product_income'), formatCurrency(ts)], [t('reports.coach_cost'), `(${formatCurrency(tc)})`]],
      [{ label: t('payments.total'), value: formatCurrency(tp + ts) }, { label: t('commissions.total'), value: formatCurrency(tc) }, { label: t('reports.net_profit'), value: formatCurrency((tp + ts) - tc) }]);
  ...
  return (
    <div className="space-y-4">
      ...
      {activeTab !== t('reports.member_detail') && activeTab !== t('reports.member_balance') && (
      ...
      {activeTab === t('reports.sales') && (
        <>
          <div className="flex justify-between items-center">
            <div className="glass-card rounded-2xl px-4 py-3 inline-flex items-center gap-2">
              <span className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40">{t('payments.total')}</span>
          ...
          <DataTable columns={[
            { key: 'transaction_date', label: t('bookings.date'), render: (r: any) => formatDate(r.transaction_date) },
            { key: 'customer_name', label: t('payments.customer') },
            { key: 'total', label: t('payments.total'), render: (r: any) => formatCurrency(r.total) },
          ]} data={sales} />
        </>
      )}

      {activeTab === t('reports.member_payments') && (
        <>
          <div className="flex justify-between items-center">
            <div className="glass-card rounded-2xl px-4 py-3 inline-flex items-center gap-2">
              <span className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40">{t('payments.total')}</span>
          ...
          <DataTable columns={[
            { key: 'payment_date', label: t('bookings.date'), render: (r: any) => formatDate(r.payment_date) },
            { key: 'member_id', label: 'Member', render: (r: any) => memberMap[r.member_id] || '-' },
            { key: 'amount', label: t('payments.amount'), render: (r: any) => formatCurrency(r.amount) },
          ]} data={payments} />
        </>
      )}

      {activeTab === t('reports.member_detail') && (
        <>
          <Select value={selectedMember} onChange={e => setSelectedMember(e.target.value)}
            options={[{ value: '', label: t('reports.select_member') }, ...members.map(m => ({ value: m.member_id, label: m.full_name }))]} />
          {selectedMember && (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <StatCard title={t('reports.booking_count')} value={bookings.length} />
                <StatCard title={t('reports.attendance')} value={bookings.filter(b => b.booking_status === 'attended').length} />
                <StatCard title={t('reports.active_packages')} value={memberPackages.filter(p => p.status === 'active').length} />
                <StatCard title={t('reports.total_paid')} value={formatCurrency(memberPayments.reduce((s, p) => s + p.amount, 0))} />
              </div>
              <Card title={t('packages.title')}>
      ...
      {activeTab === t('reports.member_balance') && (
        <>
            ...
            {allMemberPackages.map(p => (
              <div key={p.member_package_id} className="glass-card rounded-2xl p-4">
                ...
                <div className="flex justify-between items-center mt-3 pt-3 border-t border-zen-brand/5">
                  <span className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40">{t('reports.remaining_sessions')}</span>
                ...
                <div className="flex justify-between items-center mt-1">
                  <span className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40">{t('reports.expired')}</span>
                  <span className="text-xs text-zen-ink/60">{formatDate(p.expired_date)}</span>
                </div>
              </div>
            ))}
            {allMemberPackages.length === 0 && <p className="text-center text-zen-ink/30 py-8 text-sm">{t('common.no_data')}</p>}
          </div>
        </>
      )}

      {activeTab === t('reports.coach_commission') && (
        <>
          ...
          <DataTable columns={[
            { key: 'date', label: t('bookings.date'), render: (r: any) => formatDate(r.date) },
          ...
          <>
            ...
            <div className="space-y-3">
              <div className="glass-card rounded-2xl p-5">
                <div className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40 mb-1">{t('reports.net_profit')}</div>
                <div className="text-3xl font-bold tracking-tight">{formatCurrency(net)}</div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="glass-card rounded-2xl p-4 text-center">
                  <div className="text-[9px] uppercase tracking-widest font-bold text-zen-ink/40 mb-1">{t('reports.member_income')}</div>
                  <div className="text-sm font-bold">{formatCurrency(tp)}</div>
                </div>
                <div className="glass-card rounded-2xl p-4 text-center">
                  <div className="text-[9px] uppercase tracking-widest font-bold text-zen-ink/40 mb-1">{t('reports.product_income')}</div>
                  <div className="text-sm font-bold">{formatCurrency(ts)}</div>
                </div>
                <div className="glass-card rounded-2xl p-4 text-center">
                  <div className="text-[9px] uppercase tracking-widest font-bold text-zen-ink/40 mb-1">{t('reports.coach_cost')}</div>
