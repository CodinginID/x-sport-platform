...
import {
  ...
  RefreshCw, Trash2, Zap, CheckCircle2, WifiOff, ScanBarcode, Lock,
} from 'lucide-react';
...
function PrinterCard() {
  ...
  useEffect(() => {
    ...
    bt.reconnect(deviceId).then((ok) => { if (ok) setStatus('connected'); });
  ...
  const handleReconnect = async () => {
    ...
    try {
      ...
      if (ok) { setStatus('connected'); addToast(`${deviceName} terhubung kembali`, 'success'); }
      else { setStatus('disconnected'); addToast('Tidak bisa reconnect otomatis — coba "Pair Ulang"', 'warning'); }
    } catch {
      setStatus('disconnected'); addToast('Gagal reconnect', 'error');
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleTest = async () => {
    setBusy(true);
    try {
      if (!bt.isConnected()) { const dev = await bt.connect(); setDevice(dev.id, dev.name ?? 'Printer'); setStatus('connected'); }
  ...
  const handleForget = () => { bt.disconnect(); forgetDevice(); addToast('Printer dihapus', 'info'); };
    ...
    <Card title="Printer Struk">
      <div className="space-y-5">
        ...
        <div className={`rounded-3xl p-4 transition-all duration-500 ${
          ...
          : isConnecting ? 'bg-zen-brand/5 border border-zen-brand/20'
          : hasSaved ? 'bg-amber-50 border border-amber-200'
          ...
          <div className="flex items-center gap-4">
            <div className={`relative w-14 h-14 rounded-3xl flex items-center justify-center shrink-0 transition-colors ${
              ...
              {isConnected ? <BluetoothConnected size={22} className="text-white" />
                : isConnecting ? <Bluetooth size={22} className="text-white animate-pulse" />
                : hasSaved ? <WifiOff size={20} className="text-white" />
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm font-bold text-zen-ink truncate">{deviceName ?? 'Tidak ada printer'}</p>
              {isConnecting && <p className="text-[11px] text-zen-brand/70 mt-0.5 animate-pulse">Mencari perangkat...</p>}
              {!isConnecting && hasSaved && !isConnected && <p className="text-[11px] text-amber-600/70 mt-0.5">Terakhir terhubung · tap Reconnect</p>}
            </div>
            {isConnected && (
          ...
          {isConnected ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={handleReconnect} disabled={busy || isConnecting} className="col-span-2 flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-zen-brand text-white text-sm font-bold hover:bg-zen-brand/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
                {isConnecting ? <><RefreshCw size={14} className="animate-spin" /> Menghubungkan...</> : <><BluetoothConnected size={14} /> Reconnect ke {deviceName}</>}
              </button>
            ...
            <button onClick={handlePairNew} disabled={busy || isConnecting} className="w-full flex items-center justify-center gap-2.5 py-4 rounded-2xl bg-zen-brand text-white text-sm font-bold hover:bg-zen-brand/90 transition-colors disabled:opacity-60">
              {isConnecting ? <><RefreshCw size={15} className="animate-spin" /> Mencari printer...</> : <><Printer size={15} /> Scan &amp; Hubungkan Printer</>}
            </button>
          )}
        </div>
        ...
        <div className="space-y-4 pt-1 border-t border-zen-ink/5">
          <div>
            ...
            <div className="flex gap-2">
              {(['58', '80'] as PaperSize[]).map((p) => (
                <button key={p} onClick={() => setPaperSize(p)} className={`flex-1 py-2.5 rounded-2xl text-sm font-bold transition-all ${paperSize === p ? 'bg-zen-brand text-white shadow-sm' : 'bg-zen-bg text-zen-ink/50 hover:text-zen-ink'}`}>
                  {p}mm
                </button>
              ))}
            </div>
          ...
          <label className="flex items-center justify-between cursor-pointer gap-4">
            ...
            <div onClick={() => setAutoPrint(!autoPrint)} className={`relative w-11 h-6 rounded-full transition-colors shrink-0 cursor-pointer ${autoPrint ? 'bg-zen-brand' : 'bg-zen-ink/15'}`}>
...
// ─── Coming Soon Card ─────────────────────────────────────────────────────────
// Hapus komponen ini dan ganti dengan implementasi nyata saat fitur siap.

function ComingSoonCard({ icon: Icon, title, description }: { icon: React.ElementType; title: string; description: string }) {
  return (
    <div className="rounded-3xl border border-dashed border-zen-ink/10 bg-zen-bg/50 px-5 py-4 flex items-center gap-4 opacity-60">
      <div className="w-10 h-10 rounded-2xl bg-zen-ink/5 flex items-center justify-center shrink-0">
        <Icon size={18} className="text-zen-ink/30" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold text-zen-ink/50 flex items-center gap-2">
          {title}
          <span className="text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded-full bg-zen-ink/8 text-zen-ink/40">Soon</span>
        </p>
        <p className="text-[11px] text-zen-ink/30 mt-0.5 truncate">{description}</p>
      </div>
      <Lock size={13} className="text-zen-ink/20 shrink-0" />
    </div>
  );
}

// ─── Devices Section (container) ──────────────────────────────────────────────
// Tambah perangkat baru: buat Card/komponen baru di atas, lalu render di sini.
...
export function DevicesSection() {
  return (
    <div className="space-y-4">
      ...
      {/* Perangkat berikutnya: hapus ComingSoonCard dan ganti dengan komponen nyata */}
      <ComingSoonCard
        icon={ScanBarcode}
