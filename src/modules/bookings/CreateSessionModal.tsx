...
// capacity disimpan sebagai string agar bisa dikosongkan/diketik bebas; di-parse saat submit.
const emptyForm = () => ({ session_date: todayLocal(), session_time: '', capacity: '', coach_id: '', session_category: 'reguler' as 'reguler' | 'pribadi' });
...
export function CreateSessionModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const valid = form.session_date && form.session_time && capacityNum > 0;
  const submit = () => {
    if (!valid) return;
    mutation.mutate(
      { action: 'create', session: { session_date: form.session_date, session_time: form.session_time, session_category: form.session_category, capacity: capacityNum, coach_id: form.coach_id || null } },
    ...
    <Modal open={open} onClose={onClose} title="Buat Sesi">
      <div className="space-y-5">
        ...
        <div>
          ...
          <input type="number" inputMode="numeric" min={1} placeholder="mis. 10" value={form.capacity}
