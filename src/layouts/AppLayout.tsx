...
import {
  ...
  Calendar, CreditCard, PieChart, DollarSign, LogOut, MoreHorizontal, Settings,
} from 'lucide-react';
...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  const navItems = [
    ...
    { label: t('nav.members'), icon: Users, path: '/members' },
    { label: t('nav.coaches'), icon: UserCheck, path: '/coaches' },
    { label: t('nav.products'), icon: ShoppingBag, path: '/products' },
    { label: t('nav.packages'), icon: Package, path: '/packages' },
    { label: t('nav.bookings'), icon: Calendar, path: '/bookings' },
    { label: t('nav.sales'), icon: CreditCard, path: '/sales' },
    { label: t('nav.payments'), icon: CreditCard, path: '/payments' },
    { label: t('nav.commissions'), icon: DollarSign, path: '/commissions' },
    { label: t('nav.reports'), icon: PieChart, path: '/reports' },
    ...
    <div className="min-h-screen bg-zen-bg font-sans">
      ...
      <aside className="hidden lg:flex fixed top-0 left-0 z-50 h-full w-[240px] bg-white/80 backdrop-blur-xl border-r border-white/20 flex-col">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path}
                className={cn(
                  'flex items-center gap-3 px-4 min-h-[44px] rounded-2xl text-[11px] uppercase tracking-widest font-bold transition-all mb-1',
        ...
        <div className="border-t border-zen-brand/5 p-4 space-y-3">
          {user && <div className="text-[10px] uppercase tracking-widest font-bold opacity-40 truncate">{user.full_name}</div>}
          <button onClick={handleLogout} className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-red-400 hover:text-red-500 min-h-[44px] w-full transition-colors">
      ...
      {/* Main content */}
      <main className="lg:ml-[240px] pt-4 lg:pt-8 pb-24 lg:pb-8 px-4 lg:px-6">
        {children}
      </main>

      {/* Mobile bottom nav - native app style */}
      <nav className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-white/90 backdrop-blur-xl border-t border-zen-brand/5 pwa-bottom-safe">
        <div className="h-16 px-2 flex items-center justify-around">
          {primaryNav.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path}
                ...
                <item.icon size={20} />
                <span className={cn('text-[9px] mt-0.5 font-bold', active ? 'text-zen-brand' : 'text-zen-ink/30')}>{item.label}</span>
              </Link>
            );
          })}
          {/* More button */}
          <button onClick={() => setMoreOpen(true)}
            ...
            <span className="text-[9px] mt-0.5 font-bold text-zen-ink/30">Lainnya</span>
          </button>
        </div>
      </nav>

      {/* More menu - bottom sheet (native app style) */}
      {moreOpen && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden animate-slide-up">
            <div className="bg-white rounded-t-[32px] shadow-2xl px-6 pt-4 pb-8 pwa-bottom-safe">
              ...
              <div className="border-t border-zen-brand/5 pt-4 flex items-center justify-between">
                <div className="text-[10px] uppercase tracking-widest font-bold text-zen-ink/40">{user?.full_name}</div>
                <button onClick={() => { handleLogout(); setMoreOpen(false); }}
                  className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-red-400 min-h-[44px]">
